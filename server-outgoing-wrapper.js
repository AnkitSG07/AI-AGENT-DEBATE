  ...
  try {
    const payload = {
      ...
      value: {
        ...
        calls: [{ id: row.call_id, from: row.customer_phone, to: envFirst("WHATSAPP_DISPLAY_PHONE_NUMBER", "META_WHATSAPP_DISPLAY_PHONE_NUMBER"), event: row.event, timestamp: String(row.timestamp || Math.floor(Date.now() / 1000)) }]
      }
    };
...
async function graphOutboundCall({ to, sdp, sdpType, phoneNumberId }) {
  ...
  const resp = await fetch(url, { method: "POST", headers: { Authorization: `Bearer ${accessToken}`, "Content-Type": "application/json", Accept: "application/json" }, body: JSON.stringify(payload) });
  const data = await resp.json().catch(() => ({}));
  if (!resp.ok || data?.error) {
  ...
  try {
    ...
    if (bodyBuffer && String(req.originalUrl || req.url || "").split("?")[0] === "/webhook") {
      try {
        ...
        saveWebhookCallsToOdoo(bodyJson).then((r) => { if (r?.length) console.log("Odoo WhatsApp call log sync:", r); }).catch((e) => console.warn("Odoo WhatsApp call log sync failed:", e?.message || e));
      } catch {}
    }
